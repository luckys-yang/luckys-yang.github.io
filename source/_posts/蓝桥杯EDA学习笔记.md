...
在空白地方右键选择设计规则(注意此时的单位是mm还是mil，不同单位的规则是不一样的)

![](https://image-1309791158.cos.ap-guangzhou.myqcloud.com/其他/QQ截图20230718225833.webp)
...
首先在原理图里框选你的模块那在PCB里就会高亮那个模块的元器件，此时点击其中一个高亮的拖动即可拖动整个模块的元器件，这样可以快速分类

然后进行一个个模块布局，布局需要注意线最好水平或者垂直，尽量简化这样待会布线就方便

GND的话一般走顶层和底层覆铜，所以一开始可以先不看它(在左侧设备管理器里把勾去掉就看不到它的飞线了)

- 布线

一般是习惯是先信号再电源，一般如果没有大功率器件的话线宽10mil，电源线30或者50mil即可

填充式布线就是有些引脚是相连的或者IC引脚贴的很近刚刚好线宽又大就会挡到旁边引脚所以可以使用这个填充式进行布线，点4个点确定一个形状就行了
...
> 立创EDA的网络端口支持跨图页连接。目前的网络端口功能与网络标签基本一致，可以认为是不同样式而已，网络端口一般使用在有端口的地方。主要目的都是标示出网络

> 注意想要打通孔的话不能和槽孔同时存在，否则虽然在嘉立创正常显示是通的，但是在打板预览那显示是堵住的，所以方法是把通孔改成普通圆然后右键改为槽孔这样
>
> ![](https://image-1309791158.cos.ap-guangzhou.myqcloud.com/其他/QQ截图20230726235117.webp)

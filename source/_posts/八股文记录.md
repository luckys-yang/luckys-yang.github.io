...
`该电路的这一特性可用于保护 5V 的 ADC 引脚（过压保护电路）`，因为该引脚可以读取 0-5V 的电压，但如果超过 5V，齐纳二极管将不允许过压。同样，当输入电压很高时，可以使用相同的电路为负载调节 5.1V

在使用齐纳二极管设计电路时，要考虑的一件重要事情是齐纳电阻，`齐纳电阻用于限制通过齐纳二极管的电流，从而保护其免受加热和损坏，齐纳电阻的值取决于齐纳二极管的齐纳电压和额定功率`

<img src="https://image-1309791158.cos.ap-guangzhou.myqcloud.com/其他/PixPin_2024-02-28_15-57-43.webp" style="zoom:80%;" />

5. **二极管续流电路**

续流二极管基本上是一个连接在 `感性负载` 端子上的二极管，以防止在开关两端产生高压
...
信号从基极进入，从集电极输出，输入和输出共用发射极，所以称之为共射放大电路

2. 共集放大电路：同向放大电流 

随着输入信号增大，输出信号也会增大。由于输入在发射极，输出在集电极，共用基极

2. 共基放大电路：同向放大电压

- 共射极放大电路(三极管同向放大电路)

<img src="https://image-1309791158.cos.ap-guangzhou.myqcloud.com/其他/PixPin_2024-02-28_16-20-05.webp" style="zoom:80%;" />
...
`直流通路，顾名思义指的是直流电流流经的通路，用于研究静态工作点`，由于电容具有 `“隔直通交”` 的作用，所以在分析直流通路的时候，认为 `电容开路`：

![](https://image-1309791158.cos.ap-guangzhou.myqcloud.com/其他/PixPin_2024-02-28_16-21-02.webp)

`交流通路，是交流的输入信号流经的通路，用于研究动态参数`。此时认为，`（容值合适的）电容可以视为短路`，`Vcc与GND直接也视为短路`。`发射结导通以后，对于交流信号的阻抗可以忽略不计，故也视为短路`

<img src="https://image-1309791158.cos.ap-guangzhou.myqcloud.com/其他/PixPin_2024-02-28_16-23-16.webp" style="zoom:80%;" />



### 运放/比较

{% tip bolt %}运放和比较器有什么区别?{% endtip %}

- 运放主要用于线性电路，其输出可以在整个范围内连续变化。
- 比较器主要用于数字电路或开关电路，其输出是两个离散的状态之一。
- 运放通常用于需要精确放大和控制的应用，而比较器通常用于需要进行电压比较的应用

运放常用器件：LM741、LM324、TL071、OPA627 

比较器常用器件：LM339、LM393、LM311、LM741
...
> 参考文章：[AS5600磁编码器获取角度（IIC方式）的简单应用](https://blog.csdn.net/qq_44836335/article/details/134783808?ops_request_misc=&request_id=&biz_id=102&utm_term=%E7%A3%81%E7%BC%96%E7%A0%81&utm_medium=distribute.pc_search_result.none-task-blog-2~blog~sobaiduweb~default-7-134783808.nonecase&spm=1018.2226.3001.4450)

**磁铁和磁编码器的最大距离不应超过1mm，超过就无法检测出！！！**
...
![](https://image-1309791158.cos.ap-guangzhou.myqcloud.com/其他/QQ截图20231217222449.webp)

![](https://image-1309791158.cos.ap-guangzhou.myqcloud.com/其他/QQ截图20231217222544.webp)